...
coef_table = pd.DataFrame({r2 = r2_score(y_test, y_pred)print(f"Test MSE: {mse:.2f}")print(f"Test R^2: {r2:.2f}")coef_table = pd.DataFrame({    'Feature': X.columns,    'Coefficient': reg.coef_}).sort_values(by='Coefficient', key=abs, ascending=False)print(coef_table)# %% [markdown]# ```markdown# The regression coefficient for horsepower is approximately -0.03. This means that, holding all other features constant, each additional unit increase in horsepower is associated with a decrease of about 0.03 miles per gallon (mpg) in fuel efficiency.# ```# %%import numpy as npimport plotly.express as pxfig_std = px.scatter_3d(df_normalized, x='weight', y='horsepower', z='mpg',                        color='mpg', color_continuous_scale='Viridis',                        title='3D Scatter Plot of Normalized MPG, Weight, and Horsepower')fig_std.update_layout(scene=dict(    xaxis_title='Normalized Weight',    yaxis_title='Normalized Horsepower',    zaxis_title='Normalized MPG'))fig_std.update_layout(width=900, height=700)fig_std.show()318# %%
Generate
Code
Markdown
Select Kernel
